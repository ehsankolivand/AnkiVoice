...
- [ ] T001 [P] Create the deploy-test package skeleton: `tests/deploy/__init__.py` and
...
- [ ] T002 [P] Add the `en_core_web_sm` wheel-URL line to `[project.dependencies]` in
...
- [ ] T003 Run `uv lock` to regenerate `uv.lock` with the `en-core-web-sm` 3.8.0 entry (pinned URL
  + sha256); confirm `uv sync` then `uv run python -c "import en_core_web_sm"` succeeds locally.
  Commit `uv.lock`. (FR-005; depends on T002.)
- [ ] T004 [P] Write FAILING test `tests/deploy/test_uv_sync_keeps_model.py`: (a) always-on —
...
- [ ] T005 [P] Write FAILING test `tests/deploy/test_systemd_unit.py`: render
...
- [ ] T006 Create `deploy/ankivoice.service` template with `{{USER}}`/`{{INSTALL_DIR}}`
...
- [ ] T007 [P] [US1] Write FAILING unit tests in `tests/deploy/test_install_unit.py` driving
...
- [ ] T008 [P] [US1] Write FAILING test `tests/deploy/test_install_container.py::test_fresh_install`
...
- [ ] T009 [US1] Implement `install.sh` core: shebang + strict mode (`set -euo pipefail`), arg/env
  ...
  **root + Debian/Ubuntu guard** ([research D1](./research.md)), and a usage/help block. Secret-safe
  ...
  (FR-012, contract §1.)
- [ ] T010 [US1] Add the provisioning steps to `install.sh`: apt-install `ffmpeg curl
...
- [ ] T011 [US1] Add `.env` create-or-preserve to `install.sh`: if `<dir>/.env` exists, do not
  ...
  contract §3,4.)
- [ ] T012 [US1] Add the warm-up step to `install.sh`: derive the **effective** cache
...
- [ ] T013 [US1] Add unit install + start + verify to `install.sh`: render `deploy/ankivoice.service`
  ...
  SC-001; contract §8.)
- [ ] T014 [US1] Add the success summary to `install.sh`: print copy-paste next-step hints
...
- [ ] T015 [P] [US2] Extend `tests/deploy/test_install_unit.py` with a FAILING no-clobber case:
...
- [ ] T016 [P] [US2] Add `tests/deploy/test_install_container.py::test_idempotent_rerun` (skip if
...
- [ ] T017 [US2] Make `install.sh` idempotent end-to-end: every step is a safe no-op/refresh on
...
- [ ] T018 [P] [US3] Add the README "Deploy on a Debian/Ubuntu VPS" section: literal quickstart
...
- [ ] T019 [P] [US3] Update `.env.example` to document that the service uses
...
- [ ] T020 [P] [US4] Add `tests/deploy/test_install_container.py::test_uninstall` and
...
- [ ] T021 [US4] Implement `uninstall.sh`: default = stop + disable + `rm` unit + `daemon-reload`;
...
- [ ] T022 Regression gate: run `uv run pytest` and confirm the app's default suite passes with the
...
- [ ] T023 Full container proof: execute quickstart §A–H in a throwaway systemd `debian:12`
  ...
  FR-017; quickstart.)
- [ ] T024 [P] Adversarial self-review (subagents) over: installer idempotency & secret hygiene
